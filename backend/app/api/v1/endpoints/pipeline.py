...
async def trigger_collection(
    channel: Optional[str] = None, db: AsyncSession = Depends(get_db)
):
    """
    ...
    return {"message": "News collection completed", "result": result}


@router.post("/analyze")
async def trigger_analysis(limit: int = 10, db: AsyncSession = Depends(get_db)):
    """
    ...
    return {"message": "News analysis completed", "result": result}


@router.post("/post")
async def trigger_posting(limit: int = 5, db: AsyncSession = Depends(get_db)):
    """
    ...
    return {"message": "News posting completed", "result": result}
