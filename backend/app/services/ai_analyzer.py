...
from app.models import NewsItem, NewsStatus, AITask, NewsChannel
...
class AIAnalyzer:
    ...
    def create_analysis_prompt(self, title: str, content: str, channel: NewsChannel) -> str:
...
Анализируй новости о криптовалютах, блокчейне, DeFi, NFT для инвесторов и гиков."""
            insights_focus = "что это значит для инвесторов, трейдеров и крипто-сообщества"
        else:
            context = """Ты — политический аналитик и журналист. 
Анализируй новости о России, мировой политике, геополитике."""
            insights_focus = "что это значит для России, мировой политики и международных отношений"
...
Если новость НЕ релевантна теме или рейтинг < 7, верни relevance_score = 0."""
    ...
    async def call_openrouter(
        ...
        try:
            async with httpx.AsyncClient(timeout=settings.AI_TIMEOUT_SECONDS) as client:
                ...
                logger.info(f"OpenRouter API call successful, relevance: {result.get('relevance_score')}")
    ...
    async def analyze_news_item(self, news_id: str) -> bool:
        ...
        if news.status != NewsStatus.PENDING:
            logger.warning(f"News item {news_id} already analyzed, status: {news.status}")
            return False
