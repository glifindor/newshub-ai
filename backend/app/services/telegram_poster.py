...
class RateLimiter:
    ...
    async def acquire(self):
        ...
        while self.messages and (now - self.messages[0]).total_seconds() > self.time_window:
    ...
    def format_message(self, news: NewsItem, parse_mode: str = "HTML") -> str:
        ...
        if news.ai_hashtags:
            if isinstance(news.ai_hashtags, list):
                ...
                tags = [tag.strip() for tag in news.ai_hashtags.split() if tag.startswith('#')]
            else:
                tags = []
...
"""
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ Одобрить", callback_data=f"approve_{news.id}"),
                    InlineKeyboardButton("❌ Отклонить", callback_data=f"reject_{news.id}"),
                ]
            ])
    ...
    async def _send_with_retry(self, channel: str, message_text: str, image_url: Optional[str] = None):
        """
        ...
        if image_url:
            try:
                ...
                caption = message_text[:1020] + "..." if len(message_text) > 1024 else message_text
    ...
    async def _notify_admin_about_post(self, news: NewsItem, channel: str, message_id: int):
