...
from app.models import NewsSource, NewsItem, NewsStatus, NewsChannel, SourceType
...
class NewsCollector:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.crypto_keywords = [
            ...
            "blockchain", "nft", "defi", "web3", "altcoin", "token",
            "binance", "coinbase", "mining", "wallet"
        ]
        
        self.politics_keywords = [
            "kremlin", "putin", "russia", "ukraine", "sanctions",
            "moscow", "government", "president", "minister", "duma",
            "foreign policy", "diplomacy", "geopolitics"
        ]
    ...
    def filter_by_keywords(self, text: str, channel: NewsChannel) -> bool:
        ...
        keywords = (
            self.crypto_keywords if channel == NewsChannel.CRYPTO 
    ...
    async def parse_rss_feed(self, source: NewsSource) -> List[dict]:
        ...
        try:
            ...
            if feed.bozo:
                logger.warning(f"RSS feed has errors: {source.name}", error=str(feed.bozo_exception))
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(
                    f"{settings.NEWSAPI_URL}/everything",
                    params=params
                )
